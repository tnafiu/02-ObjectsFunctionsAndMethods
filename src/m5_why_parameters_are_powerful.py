...
def main():
    ...
    even_better_draw_circles(rg.Point(0, 0), 3, 10, 'red', 10)
    even_better_draw_circles(rg.Point(40, 50), 10, 8, 'blue', 5)
    k = 5
    for k in range (5):
        even_better_draw_circles(rg.Point(40, 50), 10, k, 'blue', 5)
        k = k + 5
...
def even_better_draw_circles(point, radius, num_circles, colour, thickness):
    ...
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)
    turtle.pen = rg.Pen(colour, thickness)

    for k in range(num_circles):  # k becomes 1, 2, 3, ... num_circles
